CREATE OR REPLACE FUNCTION fn_get_all_activities()
    RETURNS TABLE(activity_id integer, activity_name character varying, activity_description text, max_participants integer, group_id integer, creator_name text, activity_datetime json, location character varying, participants_count bigint, activity_type_name character varying, activity_status_name character varying, group_name character varying) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
/*
Consulta todas las actividades programadas o completadas, con información del grupo, creador, 
horarios, ubicación, tipo, estado y cantidad de participantes que asistieron.
*/
  ...
  WHERE activity_status_id IN (1,2) -- (Programada, Completada)``
  GROUP BY 
    a.activity_id,
    a.ga_activity_name,
    a.ga_activity_description,
