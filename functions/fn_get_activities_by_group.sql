...
CREATE OR REPLACE FUNCTION fn_get_activities_by_group(
	p_group_id integer)
    RETURNS TABLE(activity_id integer, activity_name character varying, activity_description text, max_participants integer, creator_name text, schedule json, location character varying, participants_count bigint, activity_type_name character varying, activity_status_name character varying, group_name character varying) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
/*
Función que retorna las actividades de un grupo específico, incluyendo detalles de la actividad,
creador, horarios en formato JSON, ubicación, cantidad de participantes con asistencia confirmada,
tipo, estado y nombre del grupo.
*/
  RETURN QUERY
  SELECT 
    a.activity_id,
  ...
  WHERE a.ga_group_id = p_group_id
  GROUP BY 
    a.activity_id,
    a.ga_activity_name,
    a.ga_activity_description,
